...
public class STBipartition {
	...
	STITreeCluster c;
	private int _hash = 0;
	//cluster c, which represents the combined cluster that includes both c1 and c2.
	...
	public STBipartition(STITreeCluster c1, STITreeCluster c2, STITreeCluster cluster) {
		if (c1.getBitSet().nextSetBit(0) > c2.getBitSet().nextSetBit(0)) {
			//The cluster with the smaller first set bit (nextSetBit(0)) is assigned to cluster1, and the other to cluster2. This ensures a consistent ordering of clusters, which is often necessary when comparing bipartitions for equality or hashing.
			cluster1 = c1;
			cluster2 = c2;
		} else {
			cluster1 = c2;
		...
		c = cluster;	
		//Avoiding duplicate representations of the same bipartition (i.e., (A|B) is treated the same as (B|A)).			
	}
	@Override
