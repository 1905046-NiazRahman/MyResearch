...
public class Posterior extends cern.jet.math.Constants{
	...
	//: Computes an incomplete beta function.
	...
	//Computes an intermediate result based on the beta ratio.
	public double r(double mi){
		double b = Math.exp(LOG2*(mi-f1)+betaRatio(mi+1,n-mi+2*lambda,f1+1,n-f1+2*lambda));
		return b;
	}
	//Combines the results of G() and r() to calculate a posterior-related value.
